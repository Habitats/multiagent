...
public class ValueProviderAgent extends Agent implements AgentReplicationHelper.Listener {
  ...
  protected void setup() {
    try {
      ...
      System.out.println("Agent " + getLocalName()
                         + " - Error retrieving AgentReplicationHelper!!! Check that the AgentReplicationService is correctly installed in this container");
      se.printStackTrace();
      doDelete();
  ...
  public void replicaAdded(AID replicaAid, Location where) {
    System.out.println(
        "Agent " + getLocalName() + " - New replica " + replicaAid.getLocalName() + " successfully added in " + where
            .getName());
  }
  ...
  public void replicaCreationFailed(AID replicaAid, Location where) {
    System.out.println(
        "Agent " + getLocalName() + " - Creation of new replica " + replicaAid.getLocalName() + " in " + where.getName()
        + " failed");
  }
