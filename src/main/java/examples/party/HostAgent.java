...
/**
 * <p> Agent representing the host for a party, to which a user-controlled number of guests is invited.  The sequence is
 * as follows: the user selects a number guests to attend the party from 0 to 1000, using the slider on the UI.  When
 * the party starts, the host creates N guest agents, each of which registers with the DF, and sends the host a message
 * to say that they have arrived.  When all the guests have arrived, the party starts.  The host selects one guest at
 * random, and tells them a rumour. The host then selects two other guests at random, and introduces them to each other.
 *  The party then proceeds as follows: each guest that is introduced to someone asks the host to introduce them to
 * another guest (at random).  If a guest has someone introduce themselves, and the guest knows the rumour, they tell
 * the other guest.  When a guest hears the rumour for the first time, they notify the host.  When all the guests have
 * heard the rumour, the party ends and the guests leave. </p> <p> Note: to start the host agent, it must be named
 * 'host'.  Thus:
 * <code><pre>
...
public class HostAgent extends Agent {
  ...
  /**
   * Setup the agent.  Registers with the DF, and adds a behaviour to process incoming messages.
   */
  ...
  /**
   * Invite a number of guests, as determined by the given parameter.  Clears old state variables, then creates N guest
   * agents.  A list of the agents is maintained, so that the host can tell them all to leave at the end of the party.
   *
  ...
  /**
   * Shut down the host agent, including removing the UI and deregistering from the DF.
   */
  ...
  /**
   * Start the conversation in the party.  Tell a random guest a rumour, and select two random guests and introduce them
   * to each other.
   */
  ...
  /**
   * Introduce guest0 to a random other guest.  Also updates the introduction count on the UI, and the avg no of
   * introductions per rumour.
   */
  ...
  /**
   * Increment the number of guests that have heard the rumour, and update the UI. If all guests have heard the rumour,
   * end the party.
   */
