...
/**
 * This agent has the following functionality: <ul> <li> registers with the DF <li> creates a list of agents <li> each
 * of this new agents registers with the DF <li> the father agent sends a message of greeting to each of them <li> it
 * waits for an answer to the greeting <li> it thanks the agents that have answered </ul>
 *
  ...
  protected void setup() {
    ...
    if (IAmTheCreator) {
      ...
      try {
        ...
        t1.start();
        System.out.println(
            getLocalName() + " CREATED AND STARTED NEW THANKSAGENT:" + t1AgentName + " ON CONTAINER " + container
                .getContainerName());
      } catch (Exception any) {
        any.printStackTrace();
      }
      ...
      try {
        ...
        t2.start();
        System.out.println(
            getLocalName() + " CREATED AND STARTED NEW THANKSAGENT:" + t2AgentName + " ON CONTAINER " + ac
                .getContainerName());
      } catch (Exception e2) {
        e2.printStackTrace();
      }
      ...
      public void action() {
        ...
        if (msg != null) {
          if (GREETINGS.equalsIgnoreCase(msg.getContent())) {
            ...
            System.out
                .println(myAgent.getLocalName() + " RECEIVED GREETINGS MESSAGE FROM " + msg.getSender().getLocalName());
